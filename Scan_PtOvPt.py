	...
	#			print(subFiles)
				...
				if 'LQ' in sample:	
					if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
						...
						df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&Muon1_Pt_over_m_muj_ak4>0.35&&abs(Muon1_Eta_m_Muon2_Eta)>0.5&&abs(Muon1_Eta_m_Muon2_Eta)<1.25&&Muon1_Pt_over_AK4Jet1_Pt>{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt))
...
# Setup TMVA
 ...
 #skip commented out lines or empty lines
	...
	histo['Significance']=TH1D('Significance','Significance',nbin,0.6,1.6)
		#print(i)
		#n_back=0
		#n_sig=0
		#n_signal=0
