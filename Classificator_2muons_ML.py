...
output = TFile.Open(opt.outFileName+'.root', 'RECREATE')
factory = TMVA.Factory(opt.outFileName, output,
...
dataloader.AddVariable('Muon2_Pt/m_muj_ak4')
dataloader.AddVariable('Muon1_Eta-Muon2_Eta')
...
#            maxentries = 0
 ...
 #           weight=(float(lumi)*float(xsec)*float(kfac))/(float(ngen))
            ...
            if 'LQ' in sample:
                ...
                #dataloader.SetSignalWeightExpression("weight_all")
                ...
                if (background['%s'%sample].GetEntries()>0):
                	...
                	dataloader.SetBackgroundWeightExpression("weight_all_2muon")
...
mycuts='Muon1_PFIso<0.05'
mycutb='Muon1_PFIso<0.05'

dataloader.PrepareTrainingAndTestTree(mycuts,mycutb,
                                      'nTrain_Signal=7000:nTrain_Background=:nTest_Signal=:nTest_Background=:SplitMode=Random:NormMode=EqualNumEvents:!V')
...
# Define model
model = nn.Sequential()
model.add_module('linear_1', nn.Linear(in_features=9, out_features=64))
...
factory.BookMethod(dataloader, TMVA.Types.kPyTorch, 'PyTorch',
                   '!H:!V:VarTransform=D,G:FilenameModel=model.pt:NumEpochs=200:BatchSize=16')
factory.BookMethod(dataloader, TMVA.Types.kBDT, 'BDT',
		    '!H:!V:NTrees=1000:MinNodeSize=5%:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.4:UseBaggedBoost:BaggedSampleFraction=0.6:SeparationType=GiniIndex:nCuts=4000')
		    #'!H:!V:NTrees=1000:MinNodeSize=5%:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=SDivSqrtSPlusB:nCuts=1000')
		    #'!H:!V:NTrees=850:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=20:IgnoreNegWeightsInTraining')
