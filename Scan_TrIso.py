	...
	#			print(subFiles)
				...
				if 'LQ' in sample:	
					if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
						...
						df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&DPhi_muj_ak4>3.08&&met_over_sq_sumEt<3&&Muon1_Pt_over_AK4Jet1_Pt>0.9&&AK4Jet1_Pt_over_m_muj_ak4>0.3&&Muon1_TrkIso<{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt))
