		...
		#			print(subFiles)
					...
					if 'LQ' in sample:	
						if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
							...
							df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&Muon1_Pt_over_AK4Jet1_Pt>0.9&&AK4Jet1_Pt_over_m_muj_ak4>0.3&&Muon2_Pt_over_m_muj_ak4<{}&&DPhi_muj_ak4>{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt,pt))
...
# Setup TMVA
 ...
 #skip commented out lines or empty lines
	...
	n_back={}
	n_sig={}
	nbin=int(0.5/float(opt.step))
	nbin2=int(0.5/0.025)
	histo['Significance']=TH2D('Significance','Significance',nbin,0.,0.5, nbin2,2.7,3.2 )
		#print(i)
		#n_back=0
		#n_sig=0
		#n_signal=0
	...
	n = pool.map(variable,np.arange(0.,0.5,float(opt.step)))
	print(n)
	pool.close()
	pool.join()
	print('###################')
