	...
	#			print(subFiles)
				...
				if 'LQ' in sample:	
					if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
						...
						df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&DPhi_muj_ak4>{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt))
...
# Setup TMVA
 ...
 #skip commented out lines or empty lines
	...
	n_back={}
	n_sig={}
	nbin=int(0.3/float(opt.step))
	histo['Significance']=TH1D('Significance','Significance',nbin,2.90,3.2)
		#print(i)
		#n_back=0
		#n_sig=0
		#n_signal=0
	...
	n = pool.map(variable,np.arange(2.90,3.2,float(opt.step)))
	pool.close()
	pool.join()
	print('###################')
	#n.reverse()
	...
	#print(i,n_sig,n_back)
		...
		if k[1]<0:
			...
			back=k[1]
			print(k[0],k[1], k[0]/(3/2+sqrt(back)),i)
