...
def variable(bdt):
	...
	np_background=[]
	#print(listSamples)
	array = np.arange(0, 1.0, 0.1)
	for j, pt in enumerate(array):
		n_signal=0
		n_background=0
		...
		#			print(subFiles)
					...
					if 'LQ' in sample:	
						if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
							...
							#df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&DPhi_muj_ak4>3.07&&met_over_sq_sumEt<4.0&&AK4Jet1_Pt_over_m_muj_ak4>{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt))
							df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&DPhi_muj_ak4>3.08&&met_over_sq_sumEt<3.0&&Muon1_Pt_over_AK4Jet1_Pt>{}&&AK4Jet1_Pt_over_m_muj_ak4>{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt,pt))
	...
	#print(n_sig['%s'%bdt],n_back['%s'%bdt], n_sig['%s'%bdt]/(0.4990/2+sqrt(n_back['%s'%bdt])))
		np_signal.append(n_signal)
		np_background.append(n_background)
		print(n_signal,n_background, pt, bdt)	
	return (np_signal,np_background)					
...
# Setup TMVA
 ...
 #skip commented out lines or empty lines
	...
	n_back={}
	n_sig={}
	nbin=int(1/float(opt.step))
	nbin2=int(1/float(opt.step))
	histo['Significance']=TH2D('Significance','Significance',nbin,0.6,1.6, nbin2,0.0,1 )
		#print(i)
		#n_back=0
		#n_sig=0
		#n_signal=0
	...
	n = pool.map(variable,np.arange(0.6,1.6,float(opt.step)))
	print(n)
	pool.close()
	pool.join()
	print('###################')
#n.reverse()
	for i,k in enumerate(n):
		#print (k[0],k[1])
		#print(k[0][0],k[1][1])
		for j,x in enumerate(k[0]):
			if k[1][j]<0:
				back=0
			else:
				back= k[1][j]
			print("dai cazzo",i,j,x,k[1][j],x/(3/2+sqrt(back)))
			histo['Significance'].SetBinContent(i+1,j+1,x/(3/2+sqrt(back)))
...
#		else:
#			back=k[1]
#			print(k[0],back, k[0]/(3/2+sqrt(back)), i,j)
#		histo['Significance'].SetBinContent(i,j,k[0]/(3/2+sqrt(back)))
...
#       print "Output file created: "+opt.outputdir+"/"+opt.outFileName+".root"
	...
	histo['Significance'].Draw("COLZ TEXT SAME")
