...
#def main():
	...
	#	BDT.append(bdt)	
def main(): 
        processes = [Process(target=variable, args=(bdt,pt)) for lineV in listSamples] 
...
# Setup TMVA
 ...
 #skip commented out lines or empty lines
	...
	n_back={}
	n_sig={}
	nbin=int(2/float(opt.step))
	histo['Significance']=TH1D('Significance','Significance',nbin,-1,1)
		#print(i)
		#n_back=0
		#n_sig=0
		#n_signal=0
	...
	n = pool.map(variable,np.arange(-1,1,float(opt.step)))
	pool.close()
	pool.join()
	print('###################')
	#n.reverse()
