	...
	#			print(subFiles)
				...
				if 'LQ' in sample:	
					if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
						...
						df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&Muon1_Pt_over_AK4Jet1_Pt>0.9&&AK4Jet1_Pt_over_m_muj_ak4>0.3&&Muon2_Pt_over_m_muj_ak4<0.4&&DPhi_muj_ak4>2.75&&abs(Muon1_Eta_m_Muon2_Eta)>0.5&&met_over_sq_sumEt<{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt))
...
# Setup TMVA
 ...
 #skip commented out lines or empty lines
	...
	n_back={}
	n_sig={}
	nbin=int(10/float(opt.step))
	histo['Significance']=TH1D('Significance','Significance',nbin,0,10)
		#print(i)
		#n_back=0
		#n_sig=0
		#n_signal=0
	...
	n = pool.map(variable,np.arange(0,10,float(opt.step)))
	pool.close()
	pool.join()
	print('###################')
	#n.reverse()
