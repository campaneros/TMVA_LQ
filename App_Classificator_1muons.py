        ...
        for subFiles in listFiles:
          #  print("adding "+subFiles)
            ...
            if 'LQ' in sample:
                ...
                df['%s'%sample]=RDataFrame(treeName,inputFile['%s'%sample])
                #df['%s'%sample]=df['%s'%sample].Snapshot(treeName,opt.outputdir+'/%s_1_muon_dataset_nodef.root'%sample)
                df['%s'%sample]=df['%s'%sample].Define("BDT",computeModel, model.GetVariableNames()).Snapshot(treeName,opt.outputdir+'/%s_1_muon_dataset.root'%sample)
                #df['%s'%sample]=df['%s'%sample].Snapshot(treeName,opt.outputdir+'/%s_1_muon_dataset_afterdef.root'%sample)
                print(sample)
...
def main():
        processes = [Process(target=variable, args=(lineS,)) for lineS in listSamples]
