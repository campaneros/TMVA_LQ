		...
		#			print(subFiles)
					...
					if 'LQ' in sample:	
						if (str(opt.mass) in sample) and (str(opt.coupling) in sample):
							...
							df['%s_%s'%(sample,bdt)]=df['%s'%sample].Filter("(m_muj_ak4>({}-{}*2*0.05))&&(m_muj_ak4<({}+{}*2*0.05))&&Muon1_Pt_over_AK4Jet1_Pt>{}&&AK4Jet1_Pt_over_m_muj_ak4>{}".format(int(opt.mass),int(opt.mass),int(opt.mass),int(opt.mass),bdt,pt))
